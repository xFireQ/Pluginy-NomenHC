...
class PermisjeCommand extends GuildCommand {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if(!$guildManager->isInGuild($nick)) {
            $sender->sendMessage("§r §7Musisz byc w gildii, aby uzyc tej komendy!");
            return;
        }
        ...
        if($rank !== "Leader" && !$guildManager->hasPermission($nick, GuildManager::PERMISSION_SET_PERMISSIONS)) {
            $sender->sendMessage("§r §7Musisz byc liderem albo posiadac permisje aby to zrobic!");
            return;
        }

        if(empty($args)) {
            $sender->sendMessage("§r §7Poprawne uzycie: /permisje §8(§2nick§8)");
            return;
        }

        if(strtolower($args[0]) == strtolower($sender->getName())) {
            $sender->sendMessage("§r §7Nie mozesz zmienic sobie permisji!");
            return;
        }

        if(!$guildManager->isInGuild($args[0])) {
            $sender->sendMessage("§r §7Nie ma tego gracza w twojej gildii!");
            return;
        }
