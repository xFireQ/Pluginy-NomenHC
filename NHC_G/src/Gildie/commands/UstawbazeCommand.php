...
class UstawbazeCommand extends GuildCommand {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if($rank !== "Leader" && $rank !== "Officer") {
            $sender->sendMessage(Main::format("§7Musisz byc liderem albo oficerem gildii aby to zrobic!"));
            return;
        }

        if(!$guildManager->isInOwnPlot($sender, $sender)) {
            $sender->sendMessage(Main::format("Nie mozesz ustawic bazy za terenem gildii!"));
            return;
        }
