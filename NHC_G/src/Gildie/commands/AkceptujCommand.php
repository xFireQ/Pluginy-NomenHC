...
class AkceptujCommand extends GuildCommand {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if(empty($args)) {
            $sender->sendMessage(Main::format("Poprawne uzycie: §9/akceptuj (tag)"));
            return;
        }
        ...
        $aLeader = $sender->getServer()->getPlayerExact($aGuild->getLeader());

        if($aLeader !== null)
            $aLeader->sendMessage(Main::format("Gildia §9{$guild->getTag()} §7zaakceptowala sojusz z twoja gildia"));

        $key = array_search(strtolower($aGuild->getTag()), Main::$alliance[$guild->getTag()]);

        unset(Main::$alliance[$guild->getTag()][$key]);

        $sender->getServer()->broadcastMessage(Main::format("Gildia §8[§9{$guild->getTag()}§8] §7zawarla sojusz z gildia §8[§9{$aGuild->getTag()}§8]"));
