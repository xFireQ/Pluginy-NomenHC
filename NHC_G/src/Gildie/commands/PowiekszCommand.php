...
class PowiekszCommand extends GuildCommand {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if(!$sender->getInventory()->contains(\pocketmine\item\ItemFactory::getInstance()->get(264, 0, 64))) {
            $sender->sendMessage("§8§l>§r §7Do przedluzenia gildii potrzebujesz §b64 §7diaxy!");
            return;
        }

        $sender->getInventory()->removeItem(\pocketmine\item\ItemFactory::getInstance()->get(264, 0, 64));

        $guild->addPlotSize(4);

        $sender->sendMessage(Main::format("Pomyslnie powiekszono teren gildii do rozmiaru §b".$guild->getPlotSize()."§7x§b".$guild->getPlotSize()));
    }
}