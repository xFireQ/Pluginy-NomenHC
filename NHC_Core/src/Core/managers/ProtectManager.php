...
class ProtectManager {
    ...
    public static function init() : void {
        Permission::setPermission("pvpcore.protect");
    ...
    public static function canBreak(Player $player, Block $block) : bool {
        if(Permission::isOp($player) || Permission::has($player, "pvpcore.protect"))
            return true;

        $nick = strtolower($player->getName());
        $postion = $block->getPosition();

        $terrainName = self::getTerrainNameFromPos($block->getPosition()->asVector3());
    ...
    public static function canInteract(Player $player, Block $block) : bool {
        ...
        if($terrainName == null) {
            $terrainName = self::getTerrainNameFromPos($player->getPosition()->asVector3());
        ...
        if(in_array($block->getId(), [54, 130, 58, 116, 145]))
            return true;

        if($block instanceof ItemFrame)
            return false;

        if(in_array($player->getInventory()->getItemInHand()->getId(), [261, 368, 46, 346]))
            return false;
    ...
    public static function addWhiteBlock(string $name, Block $block) : void {
        ...
        $pos = [$block->getPosition()->getFloorX(), $block->getPosition()->getFloorY(), $block->getPosition()->getFloorZ()];
