...
use pocketmine\item\{Item, ItemFactory, ItemIdentifier, ItemIds, ItemUseResult, Tool, VanillaItems};

class Bow extends Tool {

    public const PUNCH_COOLDOWN = 10;
    ...
    private function punch(Player $player, ArrowEntity $arrow, float $punchKnockback) : void {
        ...
        if(isset(Main::$antylogoutPlayers[$nick]) && $lastPunch < self::PUNCH_COOLDOWN) {
            $player->sendTitle("§r§l§4BLOKADA", "§r§cNie mozesz uzyc punchu!");
            $player->sendMessage("§cPuncha mozesz uzyc za §4".(self::PUNCH_COOLDOWN - $lastPunch)." §csekund");
            return;
        }
        ...
        if($punchKnockback > 0){
            ...
            if($player->isOnGround()){
                ...
                if($horizontalSpeed > 0){
                    ...
                    $player->getWorld()->broadcastLevelSoundEvent($player, LevelSoundEventPacket::SOUND_BOW);
                    $player->getWorld()->broadcastLevelSoundEvent($player, LevelSoundEventPacket::SOUND_BOW_HIT);
    ...
    public function onReleaseUsing(Player $player) : ItemUseResult {
        if($player->isSurvival() and !$player->getInventory()->contains(ItemFactory::get(\pocketmine\item\ItemIds::ARROW, 0, 1))){
        ...
        $entity = Entity::createEntity("Arrow", $player->getWorld(), $nbt, $player, $baseForce >= 1);
        if($entity instanceof Projectile){
            ...
            if($ev->isCancelled()){
                ...
                if($player->isSurvival()){
                    /*if(!$infinity){
                        $player->getInventory()->removeItem(ItemFactory::get(\pocketmine\item\ItemIds::ARROW, 0, 1));
                    }*/
                    $this->applyDamage(1);
                }
