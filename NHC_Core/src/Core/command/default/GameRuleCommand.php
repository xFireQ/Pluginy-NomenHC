...
use Core\level\gamerules\GameRules;
use Core\Main;
...
class GameRuleCommand extends Command implements PluginIdentifiableCommand {
    ...
    public function __construct() {
        parent::__construct("gamerule", "Komenda gamerule", null, []);
        $this->setPermission("core.gamerule");
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        /** @var Player $sender */
        if(is_bool($value)) {
            MultiWorld::getGameRules($sender->getWorldNonNull())->setBool($args[0], $value);
        } else {
            MultiWorld::getGameRules($sender->getWorldNonNull())->setInteger($args[0], (int)$value);
        }

        MultiWorld::getGameRules($sender->getWorldNonNull())->applyToLevel($sender->getWorldNonNull());
        $sender->sendMessage(MultiWorld::getPrefix() . LanguageManager::translateMessage($sender, "gamerule-done", [$args[0], $sender->getWorldNonNull()->getName(), $args[1]]));
    }
