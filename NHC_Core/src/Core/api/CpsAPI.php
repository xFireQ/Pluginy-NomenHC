...
class CpsAPI {
    ...
    public static function getCpsMessage(Player $player) : string {
        ...
        return "§cPrzekroczyles limit CPS §8[§4".Main::CPS_MAX."§8]§c, poczekaj §4{$seconds} §csekund!";
    }
    ...
    public static function addClick(Player $player) : void {
        ...
        if(isset(self::$blocks[$nick])) {
            ...
            $player->sendTitle("§r§l§4CPS", "§r§cPrzekroczyles limit cps!");
        }

        if(!isset(self::$blocks[$nick]) && $cps >= 13)
            self::$blocks[$nick] = time();
